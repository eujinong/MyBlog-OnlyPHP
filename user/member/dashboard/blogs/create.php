    ...
    if (Input::exists()) {
        if (Token::check(Input::get('token'))) {
            ...
            if ($objValidation->passed()) {
                ...
                $objBlog->featured_image = Input::file('featured_image');

                $inputTag = Input::get('tags');

                isset($inputTag) ? $objBlog->tags = $inputTag : $objBlog->tags = [''];

                $inserted_id = $objBlogService->submitBlog($objBlog);

                if(!$inserted_id) {
                    $message = $objErrMessage->getAlertMessage("failed to create blog!");
                } else {
                    $message = $objErrMessage->getSuccessMessage("Successfully created blog!");
                }
